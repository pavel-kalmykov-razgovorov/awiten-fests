...
<html res="{{ App::setlocale(session('lang'))}}">
    <h1 class="page-header">{{ trans('translate.newuser')}}</h1>
    @if(count($errors) > 0)
        <div class="alert alert-warning">
            <h3><span class="glyphicon glyphicon-exclamation-sign valign-top" aria-hidden="true"></span>{{ trans('translate.nocrear')}}</h3>
            <ul>
            ...
            <div class="col-md-8">
                    <form class="form-horizontal" role="form" method="POST" action="{{action('UserController@Create')}}">
                        ...
                        <div class="form-group{{ $errors->has('username') ? ' has-error' : '' }}">
                            <label for="username" class="col-md-4 control-label texto-comun">{{ trans('translate.usuario')}}</label>
